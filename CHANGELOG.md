...
**Measured:** the shipped plugin carried **289 species and 57 mega formes** against ABRA's **308 and
76**. **Nineteen mega formes were absent from the species table entirely** — starmie-mega,
meganium-mega, chimecho-mega, froslass-mega, emboar-mega, excadrill-mega and thirteen others could
not be looked up at all.

**CORRECTION, recorded rather than quietly fixed.** An earlier draft of this entry and of commit
`ced6137` claimed the plugin had "no abilities at all" and that "every mega was a stat block with no
identity". **That was wrong.** `data/mega-formes.json` carries a `by_item` stone table with the
ability for all 95 stones, and `buildMon` applies it (`if(!ab && mf.ab) ab = mf.ab`) — 75 of the 76
megas in the current dex resolve an ability through it. Mega identity was working. The error came
from testing only the first 5 KB of the `MONS` literal and generalising.

What is true: the `MONS` table itself carried no `ab` field (0 of 289), and the 306 now added there
are **inert** — `engine/champ-model.js` never reads `mon.ab`, only `set.ability` from the paste and
`mf.ab` from the stone table. They are carried for future consumers, not because anything reads them
today. The real defect fixed here is the 19 missing formes, not missing abilities.
...
32,921. **19 mega formes that could not previously be looked up are now present.** The 306 `ab`
fields are carried but inert (see the correction above). Bridge verified 6/6 after write.
